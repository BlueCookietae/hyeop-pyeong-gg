      ...
      <div className="max-w-md mx-auto px-4 pt-5 space-y-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-slate-800 overflow-hidden border border-slate-700 shrink-0">
            {info?.image ? (
              <img src={getProxiedUrl(info.image)} className="w-full h-full object-cover" alt={playerName} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
            ) : (
          ...
          <div className="flex-1 min-w-0">
            ...
            {info && (
              <div className="flex items-center gap-2 mt-1">
                {info.teamLogo && <img src={info.teamLogo.startsWith('/') ? info.teamLogo : getProxiedUrl(info.teamLogo)} className="w-4 h-4 object-contain" alt={info.teamCode} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />}
          ...
          {matchHistory.length === 0 ? (
            ...
            <div className="space-y-2">
              {matchHistory.map(m => {
                ...
                return (
                  <Link key={m.matchId} href={`/match/${m.matchId}`} className="block bg-slate-900 border border-slate-800 hover:border-slate-600 rounded-2xl p-4 transition-colors">
